...
const NavBar: FC = () => {
  ...
  return (
    <motion.div
      className="z-10 flex items-center justify-between p-4 lg:px-64"
      initial={{ y: -1024 }}
      animate={{ y: 0 }}
      transition={{ type: "spring", stiffness: 100, damping: 18 }}
    >
      <Link className="w-2/12 lg:w-[5%]" href={"/"}>
                    ...
                    {nav.data.map((item, index) => {
                      return (
                        ...
                        >
                          <Menu.Item>
                            {({ active }) => (
                              <Link className={`${active && "text-[#fff]"}`} href={item.link}>
                                {item.name}
                              </Link>
                            )}
                          </Menu.Item>
      ...
      <Popover className="text-white lg:hidden">
        ...
        <Popover.Panel>
          <motion.div
            ...
            className="fixed left-0 top-0 z-10 flex min-h-screen w-full flex-col items-center justify-center gap-10 px-4"
          >
            <Popover.Button>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                        ...
                        return (
                          <Link href={item.link} key={index}>
                            <motion.button
                              ...
                              whileTap={{ scale: 0.9 }}
                              style={{
                                backgroundColor: item.name === "Dashboard" ? color.bg2 : color.bg1,
